...
function renderReport(kind, payload) {
  ...
  const summaryHtml = summary.status === "succeeded"
    ...
    : `<div class="report-summary report-summary-muted">AI 摘要：${escapeHtml(summary.status === "blocked" ? "当前没有可用 provider 或不适用，仍保留 raw。" : summary.status === "failed" ? "AI 摘要未通过生成或证据门禁，raw 已保留；观察台与 raw 日报更新正常。" : "尚未生成，仍保留 raw。")}</div>`;
