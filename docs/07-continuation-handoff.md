...
- 在临时 PostgreSQL 15+ 数据库真实执行 `schema/postgres/001_m1_core.sql`；当前环境没有 PostgreSQL/`psql`，尚未完成 parser/transaction 验证。
- 把 ADV-013、PRI-012–013、EVA-025 物化为数据库并发、回滚和集合闭合测试。
- 生成权限矩阵、EventBase 状态机、双时间查询模板和 identifier linter。
- 物化 TestDefinition/test manifest，并证明 P0 applicability 永远 fail closed。
