...
module ReportSummaryProvider
  class Error < StandardError
    attr_reader :code
    def initialize(message, code: "report_summary_error"); @code = code; super(message); end
  end
    ...
    def summarize(input:)
      response = client.chat_json(
        ...
        system: <<~PROMPT.strip,
          ...
          顶层只能有 overview、key_changes、uncertainties。overview 是对象，后两项是数组；每个单元只能有 text 与 cited_version_ids。
          每个非空陈述必须逐字引用一个或多个本 edition 提供的短 version_id（例如 E001），不要改写、截断或自行生成 ID。区分已报道事实、来源主张、推断和不确定性；不要把来源主张改写为已证实事实。
