...
module ReportSummaryProvider
  ...
  class DeepSeek
    PROMPT_VERSION = "local-report-summary-deepseek-v4-pro-v7"
    ...
    def summarize(input:)
      response = client.chat_json(
        ...
        system: <<~PROMPT.strip,
          ...
          不要输出 claim_id；服务器会根据 edition、section、顺序、text 和 evidence_scopes 生成 canonical claim_id，任何模型提供的 claim_id 都会被服务器忽略。每个 claim 的字段只能是 kind、text、epistemic_status、evidence_scopes、以及 ai_inference 必须的 premise_scope_ids、inference_support_status。
